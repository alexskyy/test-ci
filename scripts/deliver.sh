...
OUTPUTDIR=$PWD/CbyGE/app/build/outputs/apk
DIGIOPS_DEV_APK="DigOps.Dev.apk"
DIGIOPS_REL_APK=".apk"
INNOVATION_DEV_APK="Innovation.Dev.apk"
INNOVATION_REL_APK="Innovation.Sprint.apk"
DEBUG_PATH="/dev/release/devRelease-$VERSION_NAME.*"
PROD_PATH="/prod/release/prodRelease-$VERSION_NAME.*"

RELEASE_NOTES="($BRANCH): $NUMBER"

echo -e "Release notes:\n$RELEASE_NOTES"

echo "Searching build $VERSION_NAME .. "
...
if [ -f $OUTPUTDIR/$DEBUG_PATH$INNOVATION_DEV_APK ]; then
    FULL_APP_PATH=$(ls $OUTPUTDIR/$DEBUG_PATH$INNOVATION_DEV_APK)
    ...
    FULL_APP_PATH=$(ls $OUTPUTDIR/$PROD_PATH$INNOVATION_REL_APK)
    ...
    FULL_APP_PATH=$(ls $OUTPUTDIR/$DEBUG_PATH$DIGIOPS_DEV_APK)
    ...
    FULL_APP_PATH=$(ls $OUTPUTDIR/$PROD_PATH$DIGIOPS_REL_APK)
